...
public final class NesNtsc extends VideoFilter {
    ...
    private static final boolean NES_NTSC_EMPHASIS = true;
    ...
    private static void init(final init_t impl, final nes_ntsc_setup_t setup) {
        ...
        if (gamma_size > 1) {
            final float to_float = 1.0f / (gamma_size);
    ...
    private static void merge_kernel_fields(final int[] io, int ioIndex) {
        for (int n = burst_size; n != 0; n--) {
            ...
            final int p1 = io[ioIndex + burst_size] + rgb_bias;
    ...
    private void nes_ntsc_init(final int[] ntsc, nes_ntsc_setup_t setup) {
        ...
        /* setup fast gamma */
        {
            ...
            gamma_factor = (float) Math.pow(Math.abs(gamma), 0.73f);
            if (gamma < 0) {
        ...
        for (int entry = 0; entry < nes_ntsc_palette_size; entry++) {
...

            {
                ...
                {
                    float r = y + default_decoder[0] * i
                            + default_decoder[1] * q;
                    float g = y + default_decoder[2] * i
                            + default_decoder[3] * q;
                    float b = y + default_decoder[4] * i
                            + default_decoder[5] * q;
    ...
    public static class nes_ntsc_setup_t {
        ...
        /* Basic parameters */
        final double hue;        /* -1 = -180 degrees     +1 = +180 degrees */
        final double saturation; /* -1 = grayscale (0.0)  +1 = oversaturated colors (2) */
        final double contrast;   /* -1 = dark (0.5)       +1 = light (1.5) */
        final double brightness; /* -1 = dark (0.5)       +1 = light (1.5) */
        final double sharpness;  /* edge contrast enhancement/blurring */
        /* Advanced parameters */
        final double gamma;      /* -1 = dark (1.5)       +1 = light (0.5) */
        final double resolution; /* image resolution */
        final double artifacts;  /* artifacts caused by color changes */
        final double fringing;   /* color artifacts caused by brightness changes */
        final double bleed;      /* color bleed (color resolution reduction) */
        final boolean merge_fields;  /* if true, merges even and odd fields together to 
                              reduce flicker */
        final float[] decoder_matrix; /* optional RGB decoder matrix, 6 elements */
        final int[] palette_out;  /* optional RGB palette out, 3 bytes per color */
        ...
        final int[] base_palette;/* optional 64-entry RGB palette in, 3 bytes per color */
